...
var controller = {
    ...
    deleteProject: function (req, res) {
        ...
        Project.findOneAndRemove(projectId, (err, projetcRemoved) => {
            if (err) return res.status(500).send({ message: 'Error al borrar el proyecto' });

            if (!projetcRemoved) return res.status(404).send({ message: 'No se puede eliminar ese proyecto' });

            return res.status(200).send({ project: projetcRemoved });
